...
const target = `export function calculateMetricsByDiscipline(orders: Order[], products?: any[]): Array<{
    discipline: string;
    ventas: number;
    cobranzas: number;
    moraCritica: number;
...
if (code.includes(target)) {
    fs.writeFileSync('src/lib/billing.ts', code.replace(target, replacement));
    console.log('Replaced calculateMetricsByDiscipline successfully');
} else {
    console.error('Target not found in billing.ts');
}
