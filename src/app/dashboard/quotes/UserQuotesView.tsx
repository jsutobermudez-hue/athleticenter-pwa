        ...
        <Card className="group cursor-pointer transition-all hover:border-primary/20 border-primary/5 bg-white p-4 flex flex-col gap-4 rounded-2xl shadow-sm ring-1 ring-primary/5 h-full hover:shadow-md hover:-translate-y-0.5" onClick={() => onSelect(quote)}>
            <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end gap-1">
                    <Badge className={cn("text-[7px] font-black uppercase border-none text-white px-2 h-4.5 rounded-md shadow-sm", statusColor)}>{quote.status}</Badge>
                    {isAboutToExpire && (
...
export default function UserQuotesView({ currentUser }: { currentUser: User }) {
    ...
    // METRICAS PIPELINE DE MI CARTERA
    const metrics = useMemo(() => {
        if (!rawQuotes) return { totalPipeline: 0, totalCount: 0, draftsCount: 0, activeCount: 0, convertedCount: 0, conversionRate: 0 };
        ...
        rawQuotes.forEach(q => {
            ...
            if (q.status === 'Borrador') draftsCount++;
            else if (['Enviada', 'Aceptada'].includes(q.status)) activeCount++;
            else if (q.status === 'Convertida') convertedCount++;
        });

        const conversionRate = totalCount > 0 ? (convertedCount / totalCount) * 100 : 0;
        return { totalPipeline, totalCount, draftsCount, activeCount, convertedCount, conversionRate };
    }, [rawQuotes]);
    ...
    const filteredAndSortedQuotes = useMemo(() => {
        ...
        if (term) {
            items = items.filter(q => 
                ...
                (q.customerRif || '').toLowerCase().includes(term)
            );
        }
        ...
        return items;
    }, [rawQuotes, searchTerm, statusFilter, sortBy, sortOrder]);
        ...
        <div className="flex flex-col gap-6 w-full animate-in fade-in-50 duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mx-1 sm:mx-2">
                <DashboardMetricCard 
                    ...
                    subtitle={`${metrics.totalCount} Presupuestos Total`} 
                    icon={DollarSign} 
                    iconBg="bg-blue-50" 
                ...
                />
                <DashboardMetricCard 
                    title="Borradores en Armado" 
                    value={metrics.draftsCount} 
                    subtitle="En Fase de Elaboración" 
                    icon={Save} 
                    iconBg="bg-slate-100" 
                ...
                <DashboardMetricCard 
                    ...
                    subtitle={`${metrics.convertedCount} Pedidos Ganados`} 
                    icon={Award} 
                    iconBg="bg-purple-50" 
            ...
            <Card className="border-none shadow-sm rounded-[1.8rem] sm:rounded-[2rem] bg-white overflow-hidden mx-1 sm:mx-2">
                <CardContent className="p-5 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-end">
                    <div className="space-y-1.5">
                        <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-400 tracking-widest px-1">Búsqueda Directa</Label>
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                            <Input placeholder="BUSCAR PRESUPUESTO / RIF..." className="pl-10 h-10 sm:h-11 bg-slate-50 border-none rounded-xl font-bold text-[11px] sm:text-xs uppercase shadow-inner" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>
                    </div>
                    <div className="space-y-1.5">
